...
def main():
    # Read in the CSV
    df = get_the_data_ready()

    # Check the occupancy
    testlist = check_occupancy(df)

    # Show us the magic
    plot_graph(df, testlist)


def get_the_data_ready():
    df = pd.read_csv('1679526000-1679612399-datapoint_3291.csv', sep=",")
    return df


def check_occupancy(df):
    testlist = []
    x = 0
    windowsize = 10
